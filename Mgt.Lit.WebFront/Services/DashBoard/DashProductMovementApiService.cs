...
{
    public class DashProductMovementApiService
    {
        private readonly HttpClient _http;
        private readonly AuthState _auth;
        private readonly PageContext _pageContext;

        public DashProductMovementApiService(HttpClient http, AuthState auth, PageContext pageContext)
        {
            _http = http;
            _auth = auth;
        ...
        public async Task<ProductMovementDto?> GetAsync(ProductMovementFilter filter, CancellationToken ct = default)
        {
            ...
            var url = "api/dashboard/productmovement" + (qs.Count > 0 ? "?" + string.Join("&", qs) : "");

            using var request = CreateAuthorizedGet(url);
            using var response = await _http.SendAsync(request, ct);
            response.EnsureSuccessStatusCode();
            return await response.Content.ReadFromJsonAsync<ProductMovementDto>(cancellationToken: ct);
        }
