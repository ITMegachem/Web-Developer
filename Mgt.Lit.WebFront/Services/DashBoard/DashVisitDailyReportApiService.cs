...
{
    public class DashVisitDailyReportApiService
    {
        private readonly HttpClient _http;
        private readonly AuthState _auth;

        public DashVisitDailyReportApiService(HttpClient http, AuthState auth)
        {
            _http = http;
            _auth = auth;
        }
        ...
        public async Task<VisitDailyReportDto?> GetAsync(VisitDailyReportFilter filter, CancellationToken ct = default)
        {
            ...
            var url = "api/dashboard/visitdailyreport" + (qs.Count > 0 ? "?" + string.Join("&", qs) : "");
            ...
            return await resp.Content.ReadFromJsonAsync<VisitDailyReportDto>(cancellationToken: ct);
        }
    }
}
