...
{
    public class DashCustomerOverviewApiService
    {
        private readonly HttpClient _http;
        private readonly AuthState _auth;
        private readonly PageContext _pageContext;

        public DashCustomerOverviewApiService(HttpClient http, AuthState auth, PageContext pageContext)
        {
            _http = http;
            _auth = auth;
        ...
        public async Task<CustomerOverviewDto?> GetAsync(CustomerOverviewFilter filter, CancellationToken ct = default)
        {
            ...
            var url = "api/dashboard/customeroverview" + (qs.Count > 0 ? "?" + string.Join("&", qs) : "");

            using var request = CreateAuthorizedGet(url);
            using var response = await _http.SendAsync(request, ct);
            response.EnsureSuccessStatusCode();
            return await response.Content.ReadFromJsonAsync<CustomerOverviewDto>(cancellationToken: ct);
        }
