...
{
    public class DashAverageDaysToCloseApiService
    {
        private readonly HttpClient _http;
        private readonly AuthState _auth;

        public DashAverageDaysToCloseApiService(HttpClient http, AuthState auth)
        {
            _http = http;
            _auth = auth;
        }
        ...
        public async Task<AverageDaysToCloseDto?> GetAsync(AverageDaysToCloseFilter filter, CancellationToken ct = default)
        {
            ...
            var url = "api/dashboard/averagedaystoclose" + (qs.Count > 0 ? "?" + string.Join("&", qs) : "");
            ...
            return await resp.Content.ReadFromJsonAsync<AverageDaysToCloseDto>(cancellationToken: ct);
        }
    }
}
