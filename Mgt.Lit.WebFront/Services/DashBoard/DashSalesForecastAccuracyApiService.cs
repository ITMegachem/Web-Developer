...
{
    public class DashSalesForecastAccuracyApiService
    {
        private readonly HttpClient _http;
        private readonly AuthState _auth;

        public DashSalesForecastAccuracyApiService(HttpClient http, AuthState auth)
        {
            _http = http;
            _auth = auth;
        }
        ...
        public async Task<SalesForecastAccuracyDto?> GetAsync(SalesForecastAccuracyFilter filter, CancellationToken ct = default)
        {
            ...
            var url = "api/dashboard/salesforecastaccuracy" + (qs.Count > 0 ? "?" + string.Join("&", qs) : "");
            ...
            return await resp.Content.ReadFromJsonAsync<SalesForecastAccuracyDto>(cancellationToken: ct);
        }
    }
}
